...
public class ResMgr : BaseManager<ResMgr>
{
    ...
    public void LoadAsync<T>(string path, UnityAction<T> callBack) where T: UnityEngine.Object
    {
        ...
        //要通过协同程序去异步加载资源
        //MonoMgr._instance.StartCoroutine(ReallyLoadAsync<T>(path, callBack));
    }
    ...
    public void UnloadAsset<T>(string path, bool isDel = false, UnityAction<T> callBack = null, bool isSub = true)
    {
        ...
        if(resDic.ContainsKey(resName))
        {
            ...
            //资源正在异步加载中 卸载则只取消callback
            else if(resInfo.asset == null)
            {
                //MonoMgr._instance.StopCoroutine(resInfo.coroutine);
                //resDic.Remove(resName);
                //为了保险起见 一定要让资源移除了
                //改变表示 待删除
                //resInfo.isDel = true;
